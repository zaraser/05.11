...
int main() {
    ...
    if (!input_line) {
        return 1; // Проверяем, что ввод не NULL
    }
    ft_init_shell(&shell);
    shell.input_line = ft_strtrim(input_line, " \n\t");
    ...
    printf("Trimmed: '%s'\n", shell.input_line); // Выводим обрезанную строку
    add_history(shell.input_line); // Добавляем в историю
